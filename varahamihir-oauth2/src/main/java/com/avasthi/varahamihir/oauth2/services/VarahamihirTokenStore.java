...
@Service
public class VarahamihirTokenStore implements TokenStore {
...
/*  public SanjnanTokenStore(CouchbaseAccessTokenRepository cbAccessTokenRepository, CouchbaseRefreshTokenRepository cbRefreshTokenRepository){
  ...
  public void removeAccessToken(OAuth2AccessToken oAuth2AccessToken) {
    ...
    if (accessToken.isPresent()) {
      CouchbaseAccessToken couchbaseAccessToken = accessToken.get();
      Optional<CouchbaseRefreshToken> refreshTokenOptional = cbRefreshTokenRepository.findByTokenId(couchbaseAccessToken.getRefreshToken());
      if (refreshTokenOptional.isPresent()) {
        cbRefreshTokenRepository.delete(refreshTokenOptional.get());
      }
  ...
  public void storeRefreshToken(OAuth2RefreshToken refreshToken, OAuth2Authentication authentication) {
    ...
    crt.setId(UUID.randomUUID().toString());
